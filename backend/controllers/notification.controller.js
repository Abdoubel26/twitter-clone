...
export const seeNotifs = async (req, res) => {
    const { id } = req.id
    try {
        const updatednotifs = Notification.updateMany({to: id}, {seen: true}, { new: true})
        return res.status(200).json({ success: true, message: "Notifications seen", notifications: updatednotifs})
    } catch(e) {
        return res.status(500).json({ success: false, message:`Server Error: ${e.message}`})
    }
}
